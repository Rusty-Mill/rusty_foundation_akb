...
The [graphics and presentation analysis](../02-capabilities/graphics/README.md) defines workload-contract device selection, device epochs, memory/resources, explicit submission/synchronization, bounded frame pacing, and presentation over window-owned surface generations. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) composes the infrastructure while preserving explicit renderer/UI, text, input, accessibility, and lifecycle gaps. Next gates are input provenance/IME routing and portable text/font/layout contracts; a general graphics command model remains deferred until multiple renderer workloads justify it.
