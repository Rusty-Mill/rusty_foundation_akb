...
The [notifications and user-attention foundations analysis](../02-capabilities/notifications/README.md) defines typed localized content, evidence-scoped submission milestones, user-controlled attention, untrusted action activation, replacement/progress/badges, scheduling, privacy/accessibility, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.19.0 composes notifications conditionally while preserving explicit remote push, alarm/call/emergency, preview/recording/codec/container/storage, renderer/UI, editing, application-protocol/plugin-interface, and product-policy/content gaps. Next gate is power and energy-management foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
