...
Next gate is service discovery, traffic routing, and load-balancing foundations: service/endpoint identity and generations, registration and leases, DNS/native/control-plane discovery, health/readiness and outlier evidence, client/server/proxy balancing, locality/fault domains, weighted subsets and progressive delivery, connection/session affinity, retries/hedges/load shedding, circuit breaking, failover/failback, security/privacy, configuration propagation, observability, conformance, and benchmarks. A general graphics command model remains deferred until multiple renderer workloads justify it.
