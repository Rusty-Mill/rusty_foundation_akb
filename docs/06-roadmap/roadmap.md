...
The [time-based media foundations analysis](../02-capabilities/time-media/README.md) defines bounded source/container/track models, exact timeline domains, generation-scoped codec/raw resources, playback clocks and A/V/text synchronization, negotiated seek/buffer discontinuities, timed text/accessibility, encode/mux/recording, protection boundaries, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.25.0 composes time-based media conditionally while preserving explicit adaptive streaming, DRM/license, editing, application player UX/policy, content authenticity, renderer/UI, federation/protocol credentials, remote push/application protocols, privileged display/system-power control, plugin interfaces, and product-content gaps. Next gate is application activation, URI/file association, and default-handler foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
