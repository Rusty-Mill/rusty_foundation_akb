...
The [internationalization and localization analysis](../02-capabilities/internationalization/README.md) defines locale preference/context snapshots, resource and typed-message resolution, formatting/parsing, calendar/time-zone ambiguity and versioning, collation/search, pseudolocalization, live changes, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.6.0 composes window, presentation, input, text, accessibility, transfer, and i18n infrastructure while preserving explicit renderer/UI, editing, product-semantic/importer/translation, and lifecycle gaps. Next gates are configuration/change-notification and observability/crash diagnostics; a general graphics command model remains deferred until multiple renderer workloads justify it.
