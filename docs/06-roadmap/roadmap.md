...
The [secure transport and channel foundations analysis](../02-capabilities/networking/secure-transport-README.md) defines exact TLS/QUIC policy, original-service authentication, client credentials, ALPN/SNI/ECH negotiation, bounded async handshakes/readiness, protected data/truncation/closure, new-channel resumption, separately authorized replayable early data, exporters/channel binding, QUIC streams/datagrams/migration, conformance, and staged benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.35.0, [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.7.0, [repository-operator profile](../02-capabilities/profiles/foundation-repository-operator.md) version 0.3.0, and [CA-operator profile](../02-capabilities/profiles/foundation-ca-operator.md) version 0.2.0 compose client/operator channels while preserving application protocols and authorization. Next gate is HTTP/1.1, HTTP/2, and HTTP/3 client/server foundations covering messages, streaming, multiplexing, proxying, caching, redirects, authentication, retries, cancellation, and observability; a general graphics command model remains deferred until multiple renderer workloads justify it.
