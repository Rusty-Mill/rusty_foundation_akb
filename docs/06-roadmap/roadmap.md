...
The [networking foundations analysis](../02-capabilities/networking/README.md) defines service identities/endpoints, expiring resolution candidate sets, connection racing, streams, datagrams, listeners, path observation, secure-channel composition, authority, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.10.0 composes window, presentation, input, text, accessibility, transfer, i18n, configuration, observability, lifecycle, and conditional networking infrastructure while preserving explicit renderer/UI, editing, application-protocol, and product-semantic/importer/translation/restoration gaps. Next gates are memory/mapping foundations and plugin/module lifecycle; a general graphics command model remains deferred until multiple renderer workloads justify it.
