...
The [distributed coordination and consistency foundations analysis](../02-capabilities/coordination/README.md) defines revisioned membership/failure evidence, leases with resource-enforced fencing, elections/locks/semaphores/barriers, consensus and replicated-state boundaries, precisely checked consistency histories, distributed transactions and sagas, reconfiguration/recovery, cross-cutting qualities, conformance, and staged fault benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.39.0, [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.11.0, [repository-operator profile](../02-capabilities/profiles/foundation-repository-operator.md) version 0.7.0, and [CA-operator profile](../02-capabilities/profiles/foundation-ca-operator.md) version 0.6.0 compose coordination conditionally while preserving algorithms/providers/topologies, state machines, fault models, and domain effects. Next gate is application data persistence and database foundations: typed records/keys and query plans, transactions/isolation, indexes and constraints, connection/session/pooling, migrations/schema compatibility, change feeds, encryption/privacy, backup/restore/PITR, replication/failover boundaries, observability, conformance, and benchmarks. A general graphics command model remains deferred until multiple renderer workloads justify it.
