...
The [accessibility foundations analysis](../02-capabilities/accessibility/README.md) defines application-owned semantic trees, text ranges, focus/actions/events/live updates, virtualization, user preferences, privacy, native adapter mappings, assistive-technology conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.4.0 composes window, presentation, input, text, and accessibility infrastructure while preserving explicit renderer/UI, editing, product-semantic, and lifecycle gaps. Next gates are clipboard/drag-and-drop data transfer and localization/formatting; a general graphics command model remains deferred until multiple renderer workloads justify it.
