...
The [application services, background execution, and durable scheduling foundations analysis](../02-capabilities/background-services/README.md) defines immutable service/job definitions, transactional native registration, system/user/session scopes, demand activation/readiness, exact schedule time domains and missed-work policy, trigger reconciliation, execution attempts/checkpoints/retries, resource policy, generation updates, security/accessibility, conformance, and benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.29.0 and [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.1.0 compose these services conditionally while preserving explicit package distribution, application protocols, database/queue/distributed coordination, domain work policy, and exactly-once effect gaps. Next gate is cryptographic operations and key-management foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
