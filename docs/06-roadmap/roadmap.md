...
The [asynchronous I/O foundations analysis](../02-capabilities/async-io/README.md) defines completion-oriented operations across completion/readiness/blocking providers, exact terminal progress, cancellation lifetime, resource/registration generations, bounded backpressure/fairness, engine/executor separation, observability, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.16.0 composes this infrastructure conditionally while preserving explicit class-protocol, renderer/UI, editing, codecs/media graphs, application-protocol/plugin-interface, and product-policy/content gaps. Next gate is storage-volume and removable-media foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
