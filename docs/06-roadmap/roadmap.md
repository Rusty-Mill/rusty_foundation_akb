...
The [certificate, trust-store, and PKI-validation foundations analysis](../02-capabilities/security/pki-README.md) defines bounded immutable certificate evidence, generation-scoped anchors/distrust/purpose policy, untrusted candidate path construction, exact purpose/time/identity-bound validation, revocation/status freshness, privacy-bounded network/cache behavior, evidence-rich results/overrides/lifecycle, conformance, and staged benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.31.0 and [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.3.0 compose trust conditionally while preserving explicit certificate issuance/enrollment/renewal, protocol proof-of-possession, transparency operation, code/document/package signing, and authorization gaps. Next gate is signed artifact, code/document/package signing, and provenance foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
