...
The [input foundations analysis](../02-capabilities/input/README.md) separates physical/logical keyboard observations from text composition/commit, defines pointer/touch streams, focus/capture routing, event provenance, privacy, loss/reset, conformance, and latency evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.2.0 composes window, presentation, and input infrastructure while preserving explicit renderer/UI, editing/text layout, accessibility, and lifecycle gaps. Next gates are portable text/font/layout contracts and semantic accessibility integration; a general graphics command model remains deferred until multiple renderer workloads justify it.
