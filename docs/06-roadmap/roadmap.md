...
The [cryptographic operations and key-management foundations analysis](../02-capabilities/security/crypto-README.md) defines versioned algorithm policy, immutable plans, opaque operation-scoped key generations, hash/MAC/KDF/password derivation, AEAD, signatures/verification/agreement, bounded import/export/wrapping, providers/hardware/attestation/certification, operation lifecycle, conformance vectors, and benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.30.0 and [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.2.0 compose cryptography conditionally while preserving explicit protocol/PKI/certificate, code/document signing, remote vault/HSM, backup/escrow, and product-policy gaps. Next gate is certificate, trust-store, and PKI validation foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
