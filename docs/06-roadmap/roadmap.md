...
The [application activation and association foundations analysis](../02-capabilities/activation/README.md) defines typed intents, handler/default association snapshots, brokered file/URI/application/reveal activation, incoming instance routing, explicit file authority and URI safety, packaging registration, boundary-scoped milestones, privacy/accessibility, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.26.0 composes activation conditionally while preserving explicit shell/process execution, default-setting, workflow acknowledgment, remote push/application protocols, adaptive media/licensing, editing/UI, federation, privileged display/system-power control, plugin interfaces, and product-policy/content gaps. Next gate is screen and window capture foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
