...
The [application messaging and RPC foundations analysis](../02-capabilities/messaging/README.md) defines typed interaction kinds, directional schema/envelope evolution, unary and streaming RPC, deadline/cancellation/status, broker delivery/settlement and pub/sub boundaries, ordering, retry/hedging/redelivery, idempotency/deduplication, inbox/outbox and reconciliation, cross-cutting qualities, conformance, and staged benchmarks across IPC, HTTP, and real-time bindings. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.38.0, [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.10.0, [repository-operator profile](../02-capabilities/profiles/foundation-repository-operator.md) version 0.6.0, and [CA-operator profile](../02-capabilities/profiles/foundation-ca-operator.md) version 0.5.0 compose messaging conditionally while preserving exact IDLs, brokers, workflows, transactions, and effect claims. Next gate is distributed coordination and consistency foundations: membership and failure evidence, leases/fencing, leader election, distributed locks/semaphores, consensus and consistency claims, replicated state, clocks/partitions, transactions/sagas, failover/recovery, observability, conformance, and benchmarks. A general graphics command model remains deferred until multiple renderer workloads justify it.
