...
The [plugin and module lifecycle analysis](../02-capabilities/plugins/README.md) defines package identity/provenance, metadata-only discovery, interface compatibility, isolation/authority, activation/quiescence, immutable generation updates, rollback, supply-chain evidence, conformance, and benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.12.0 composes the prior infrastructure with optional plugin hosting while preserving explicit renderer/UI, editing, application-protocol/plugin-interface, and product-semantic/importer/translation/restoration gaps. Next gates are threading/synchronization and audio/media foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
