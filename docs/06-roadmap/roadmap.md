...
The [signed-artifact and provenance foundations analysis](../02-capabilities/signed-artifacts/README.md) defines exact versioned signed views, declared artifact identity and intent, authority-bearing signing ceremonies, native/portable envelopes, trusted timestamps, transparency, provenance/SBOM/reproducibility evidence, acceptance policy, lifecycle, conformance, and staged benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.32.0 and [server foundation profile](../02-capabilities/profiles/foundation-server.md) version 1.4.0 compose verification and signing conditionally while preserving explicit installation/execution/publication authority and package/update policy. Next gate is software package installation, authenticated update manifests, dependency/conflict planning, staged rollout, rollback, and recovery foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
