...
The [device discovery and hardware-change foundations analysis](../02-capabilities/devices/README.md) defines scoped queries, coherent snapshots, generation-bound identity evidence, typed/provenanced properties and topology, notification-driven reconciliation, privacy, class handoff, conformance, and scale/convergence benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.15.0 composes conditional general-device observation while preserving explicit class-protocol, renderer/UI, editing, codecs/media graphs, application-protocol/plugin-interface, and product-policy/content gaps. Next gate is asynchronous I/O architecture; a general graphics command model remains deferred until multiple renderer workloads justify it.
