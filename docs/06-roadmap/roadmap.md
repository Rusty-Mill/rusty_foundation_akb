...
The [configuration and change-notification analysis](../02-capabilities/configuration/README.md) defines typed schemas and source plans, provenance-bearing immutable snapshots, validation/reload policy, secret-reference boundaries, native invalidation reconciliation, loss/resynchronization, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.7.0 composes window, presentation, input, text, accessibility, transfer, i18n, and configuration infrastructure while preserving explicit renderer/UI, editing, product-semantic/importer/translation, observability, and lifecycle gaps. Next gates are observability/crash diagnostics; a general graphics command model remains deferred until multiple renderer workloads justify it.
