...
The [remote presentation and controlled input foundations analysis](../02-capabilities/remote-interaction/README.md) composes selected capture with separately secured presentation transport, participant/session/action-scoped grants, revision-bound coordinates/keymaps, untrusted remote input, bounded native injection, explicit device-state cleanup, local precedence, accessible indication and emergency revocation, conformance, and end-to-end latency evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.28.0 composes remote interaction conditionally while preserving explicit codec/signaling/NAT traversal, clipboard/file transfer, elevation, unattended access, renderer/UI, editing, adaptive media/licensing, federation, privileged display/system-power, plugin-interface, and product-policy gaps. Next gate is application services, background execution, and durable scheduling foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
