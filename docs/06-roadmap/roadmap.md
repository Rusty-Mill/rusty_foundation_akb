...
The [printing and document-output foundations analysis](../02-capabilities/printing/README.md) defines destination/capability generations, immutable paginated sources, format-bound whole-ticket negotiation, bounded rendering/color, boundary-scoped job evidence, artifact output, privacy/accessibility, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.22.0 composes printing conditionally while preserving explicit accessible-document authoring, editing, renderer/UI, codecs/media, federation/protocol credentials, remote push/application protocols, privileged system-power control, physical attestation, plugin interfaces, and product-policy/content gaps. Next gate is display and color-management foundations beyond topology; a general graphics command model remains deferred until multiple renderer workloads justify it.
