...
The [text, fonts, and layout analysis](../02-capabilities/text/README.md) defines typed revisioned Unicode positions, exact font plans, deterministic shaping and cluster maps, bidi/line layout, caret/hit-test geometry, rasterization, multilingual conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.3.0 composes window, presentation, input, and text infrastructure while preserving explicit renderer/UI, editing, accessibility-adapter, and lifecycle gaps. Next gates are semantic accessibility/platform adapters and clipboard/drag-and-drop data transfer; a general graphics command model remains deferred until multiple renderer workloads justify it.
