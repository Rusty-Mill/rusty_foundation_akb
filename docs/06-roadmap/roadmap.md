...
The [application lifecycle and session-integration analysis](../02-capabilities/lifecycle/README.md) defines instance epochs, readiness, typed activation, multi-instance policy, session/power observations, cooperative termination, scoped inhibition, restoration metadata, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.9.0 composes window, presentation, input, text, accessibility, transfer, i18n, configuration, observability, and lifecycle infrastructure while preserving explicit renderer/UI, editing, product-semantic/importer/translation/restoration, and networking gaps. Next gate is networking foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
