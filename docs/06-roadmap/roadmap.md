...
The [memory and mapping foundations analysis](../02-capabilities/memory/README.md) defines virtual address/backing state, file/shared mappings, protection, residency/discard, allocator boundaries, executable-memory authorization, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.11.0 composes the prior infrastructure with optional explicit memory/mapping support while preserving explicit renderer/UI, editing, application-protocol, and product-semantic/importer/translation/restoration gaps. Next gate is plugin/module lifecycle; a general graphics command model remains deferred until multiple renderer workloads justify it.
