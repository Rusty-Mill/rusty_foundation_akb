...
The [credential and identity-session foundations analysis](../02-capabilities/identity-session/README.md) separates provider-scoped principals, login-session generations, security-context snapshots, authentication evidence, opaque credential handles, authority, and restricted delegation/impersonation. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.21.0 composes local identity/session behavior conditionally while preserving explicit federation/account lifecycle, protocol credentials, remote push, renderer/UI, editing, codecs/media, privileged system-power control, application-protocol/plugin-interface, and product-policy/content gaps. Next gate is printing and document-output foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
