...
The [windowing foundations analysis](../02-capabilities/windowing/README.md) defines negotiated window lifecycle, atomic revisioned configuration, display topology, typed coordinate spaces, and generation-scoped presentation surfaces. Accepted ADRs keep graphics/presentation mechanics outside windowing and reject global-coordinate/DPI assumptions. Next gates are graphics device/presentation contracts, followed by input event provenance and text/layout integration.
