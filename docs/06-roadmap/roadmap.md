...
The [observability, diagnostics, and crash-reporting analysis](../02-capabilities/observability/README.md) defines stable structured schemas, explicit causal/time context, bounded metrics/tracing pipelines, exporter independence, diagnostic bundles, privacy governance, loss accounting, and separated minimal crash capture/symbolication. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.8.0 composes window, presentation, input, text, accessibility, transfer, i18n, configuration, and observability infrastructure while preserving explicit renderer/UI, editing, product-semantic/importer/translation, and application-lifecycle gaps. Next gates are application lifecycle/session integration and networking foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
