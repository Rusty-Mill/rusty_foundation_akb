...
The [power and energy-management foundations analysis](../02-capabilities/power/README.md) defines qualified battery/source/saver/thermal observations, explicit workload adaptation, bounded sleep/display assertion leases, lifecycle reconciliation, energy measurement, privacy/accessibility, conformance, and sustained benchmarks. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.20.0 composes power behavior conditionally while preserving explicit system transition/wake/device-control, remote push, renderer/UI, editing, codecs/media, application-protocol/plugin-interface, and product-policy/content gaps. Next gate is credential and identity-session foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
