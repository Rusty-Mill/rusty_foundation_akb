...
Next gate is search, indexing, and retrieval foundations: document identity/versioning, schema/mappings/analyzers, ingestion and change capture, commit/refresh visibility, inverted/vector/spatial indexes, query and ranking semantics, pagination/cursors, filters/facets/highlighting, multi-tenancy/privacy, index migration/rebuild, replication/recovery, relevance evaluation, observability, conformance, and benchmarks. A general graphics command model remains deferred until multiple renderer workloads justify it.
