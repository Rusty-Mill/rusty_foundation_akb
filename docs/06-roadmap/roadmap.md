...
The [display and color-management foundations analysis](../02-capabilities/display-color/README.md) defines semantic image descriptions, revisioned display color evidence, generation-scoped compositor negotiation, conversion/gamut/tone mapping, profile/calibration/measurement boundaries, lifecycle, privacy/accessibility, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.23.0 composes color-managed presentation conditionally while preserving explicit calibration/control, appearance attestation, accessible-document authoring, editing, renderer/UI, codecs/media, federation/protocol credentials, remote push/application protocols, privileged system-power control, plugin interfaces, and product-policy/content gaps. Next gate is image codec and still-image foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
