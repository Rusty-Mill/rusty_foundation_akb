...
The [camera and media-capture foundations analysis](../02-capabilities/capture/README.md) defines permission-separated camera observation, revocable capture authority, negotiated session generations, exact raw frame/color/orientation layouts, clocks, controls, bounded delivery, privacy/accessibility, conformance, and performance evidence. The [windowed desktop foundation profile](../02-capabilities/profiles/foundation-windowed-desktop.md) version 0.18.0 composes capture conditionally while preserving explicit preview/recording/codec/container/storage, formatting/partitioning/repair/unlock, renderer/UI, editing, application-protocol/plugin-interface, and product-policy/content gaps. Next gate is notifications and user-attention foundations; a general graphics command model remains deferred until multiple renderer workloads justify it.
