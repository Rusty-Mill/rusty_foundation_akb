...
[`rm.profile.foundation.windowed-desktop` 0.37.0](foundation-windowed-desktop.md) extends Desktop 1.0.0 with negotiated top-level windowing, graphics/color-managed presentation, focused input/text composition, exact text infrastructure, semantic accessibility/native adapters, data transfer, explicit localization/formatting, typed snapshot-based configuration, bounded structured observability, application lifecycle/session integration, conditional networking/TLS-QUIC/HTTP/real-time-transport/audio/camera/screen-capture/remote-interaction/background-service/cryptographic/certificate-trust/enrollment/signed-artifact/package-deployment/notifications/power/general device/identity-session/printing/still-image/time-media/activation observation and completion-oriented async I/O/storage-volume workflows, optional scoped reauthentication and delegated credential operations, optional explicit memory/mapping, optional isolated plugins, and explicit native-thread/synchronization foundations. It remains intentionally incomplete until application subprotocols/schemas/durable messaging, renderer/UI, editing and accessible-document authoring, adaptive streaming/license/application media policy, public-trust/CA and remote-vault policy, repository publication/operations and vulnerability policy, signaling/NAT traversal/unattended-access policy, platform push/workflow acknowledgments, account/federation protocols, privileged display/system-power control, translated product resources/semantics/importers, plugin interfaces, and product-specific restoration/audio/capture/remote-control/background-work/crypto/trust/enrollment/signing/deployment/notification/power/device/storage/identity/printing/color/image/media/activation/HTTP/real-time policy are selected.

[`rm.profile.foundation.server` 1.9.0](foundation-server.md) conditionally selects OS-managed service registration/demand activation, durable scheduling, cryptographic policy/key operations, certificate trust/path validation and enrollment/renewal, TLS/QUIC secure channels, HTTP/1.1-/2-/3 client/server exchange, WebSocket/SSE/versioned-WebTransport sessions, signed-artifact verification/production, and package deployment/update orchestration when workload predicates require them. It preserves explicit routes/media types/schemas/subprotocols, durable messaging and domain acknowledgments, CA/public-trust and repository policy, database/queue/distributed coordination, deployment topology, remote vault/HSM protocol, exactly-once domain effect, and operational gaps.

## Repository operator extension

[`rm.profile.foundation.repository-operator` 0.5.0](foundation-repository-operator.md) extends Server 1.9.0 with conditional release publication, authenticated repository metadata, channels, mirrors, advisory/disclosure, revocation/emergency, retention/backup, service credentials, secure-channel/HTTP and optional real-time observation evidence, conformance, and operational evidence. It leaves routes/media types/schemas/subprotocols, storage/coordination topology, interchange profiles/providers, service objectives, legal workflow, and staffing to product RFCs.

## Certificate-authority operator extension

[`rm.profile.foundation.ca-operator` 0.4.0](foundation-ca-operator.md) extends Server 1.9.0 with registration/validation, issuance policy and ledger, CA key ceremonies, status/transparency, renewal/revocation, hierarchy rotation, compromise/termination, secure-channel/HTTP and optional real-time observation evidence, recovery, conformance, and benchmarks. It leaves wire mappings/subprotocols, trust model, certificate policies/profiles, providers, topology, legal/audit regime, service objectives, and high-risk server-key/escrow/public-trust features explicit.
