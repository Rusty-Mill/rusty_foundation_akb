...
[`rm.profile.foundation.windowed-desktop` 0.39.0](foundation-windowed-desktop.md) extends Desktop 1.0.0 with negotiated windowing/graphics/input/text/accessibility, data transfer, i18n/configuration/observability/lifecycle, and conditional networking/TLS-QUIC/HTTP/real-time/messaging/distributed-coordination/audio/capture/background/security/deployment/device/document/media/activation capabilities. It retains optional reauthentication/delegation, memory, plugins, and explicit threading foundations. It remains incomplete until product schemas/brokers/coordination providers/topologies/fault models/domain state and effects, UI/document/media policy, trust/repository/account/push/signaling policy, translations/importers, plugin interfaces, and other product-specific choices are selected.

[`rm.profile.foundation.server` 1.11.0](foundation-server.md) conditionally selects managed services/schedules, cryptographic/certificate lifecycle, TLS/QUIC, HTTP, real-time sessions, messaging/RPC, distributed coordination/consistency, signed artifacts, and deployment. It preserves exact IDLs/providers/topologies/algorithms, databases/state machines, fault domains and sizing, domain transactions/workflows/effects, CA/repository policy, remote vault protocols, and operational gaps.

## Repository operator extension

[`rm.profile.foundation.repository-operator` 0.7.0](foundation-repository-operator.md) extends Server 1.11.0 with conditional publication, authenticated metadata, channels/mirrors/advisories/emergency/retention, credentials, and optional HTTP/real-time/messaging/coordination composition evidence. It leaves routes/IDLs/schemas/brokers, storage and coordination algorithms/topology, interchange providers, objectives, legal workflow, and staffing to product RFCs.

## Certificate-authority operator extension

[`rm.profile.foundation.ca-operator` 0.6.0](foundation-ca-operator.md) extends Server 1.11.0 with registration/validation, issuance ledger/policy, key ceremonies, status/transparency, renewal/revocation, hierarchy/compromise lifecycle, and optional HTTP/real-time/messaging/coordination composition evidence. It leaves mappings/IDLs/schemas/brokers, coordination algorithms/topology, trust/certificate policy, providers, legal/audit regime, objectives, and high-risk features explicit.
