...
[`rm.profile.foundation.windowed-desktop` 0.6.0](foundation-windowed-desktop.md) extends Desktop 1.0.0 with negotiated top-level windowing, graphics presentation, focused input/text composition, exact text infrastructure, semantic accessibility/native adapters, data transfer, and explicit localization/formatting services. It remains intentionally incomplete until renderer/UI, editing, translated product resources/semantics/importers, and lifecycle contracts are selected.
...
These profiles are incomplete for real applications because terminal, networking, configuration, observability, rendering/UI, editing, product-specific accessible semantics/data importers/translations, and audio contracts are not yet all specified. The windowed extension supplies window/presentation/input/text/accessibility/transfer/i18n infrastructure but not those product layers. Resolution reports must preserve the gaps; the foundation profile name prevents a false completeness claim.
