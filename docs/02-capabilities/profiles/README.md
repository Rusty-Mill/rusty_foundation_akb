...
[`rm.profile.foundation.windowed-desktop` 0.36.0](foundation-windowed-desktop.md) extends Desktop 1.0.0 with negotiated top-level windowing, graphics/color-managed presentation, focused input/text composition, exact text infrastructure, semantic accessibility/native adapters, data transfer, explicit localization/formatting, typed snapshot-based configuration, bounded structured observability, application lifecycle/session integration, conditional networking/TLS-QUIC/HTTP/audio/camera/screen-capture/remote-interaction/background-service/cryptographic/certificate-trust/enrollment/signed-artifact/package-deployment/notifications/power/general device/identity-session/printing/still-image/time-media/activation observation and completion-oriented async I/O/storage-volume workflows, optional scoped reauthentication and delegated credential operations, optional explicit memory/mapping, optional isolated plugins, and explicit native-thread/synchronization foundations. It remains intentionally incomplete until other application protocols, renderer/UI, editing and accessible-document authoring, adaptive streaming/license/application media policy, public-trust/CA and remote-vault policy, repository publication/operations and vulnerability policy, signaling/NAT traversal/unattended-access policy, remote push/workflow acknowledgments, account/federation protocols, privileged display/system-power control, translated product resources/semantics/importers, plugin interfaces, and product-specific restoration/audio/capture/remote-control/background-work/crypto/trust/enrollment/signing/deployment/notification/power/device/storage/identity/printing/color/image/media/activation/HTTP policy are selected.

[`rm.profile.foundation.server` 1.8.0](foundation-server.md) conditionally selects OS-managed service registration/demand activation, durable scheduling, cryptographic policy/key operations, certificate trust/path validation and enrollment/renewal, TLS/QUIC secure channels, HTTP/1.1-/2-/3 client/server exchange, signed-artifact verification/production, and package deployment/update orchestration when deployment, work-survival, data protection, peer authentication, managed credentials, secure networking, HTTP, artifact publication/acceptance, or servicing requirements need them. It preserves explicit HTTP routes/media types/schemas, other application protocols, CA operation/public-trust policy, repository operations, vulnerability/advisory policy, database/queue, distributed coordination, deployment topology, remote vault/HSM protocol, exactly-once domain effect, and operational policy gaps.

## Repository operator extension

[`rm.profile.foundation.repository-operator` 0.4.0](foundation-repository-operator.md) extends Server 1.8.0 with conditional release publication, authenticated repository metadata, channels, mirrors, advisory/disclosure, revocation/emergency, retention/backup, service-credential enrollment/renewal, secure-channel and optional HTTP composition evidence, conformance, and operational evidence. It intentionally leaves HTTP routes/media types/schemas and other application APIs, storage/coordination topology, concrete interchange profiles/providers, service objectives, legal workflow, and staffing to product RFCs.

## Certificate-authority operator extension

[`rm.profile.foundation.ca-operator` 0.3.0](foundation-ca-operator.md) extends Server 1.8.0 with registration/validation, issuance policy and ledger, CA key ceremonies, status/transparency, renewal/revocation, hierarchy rotation, compromise/termination, secure-channel and optional HTTP composition evidence, recovery, conformance, and benchmarks. It leaves enrollment/status/administration wire mappings, trust model, certificate policies/profiles, providers, topology, legal/audit regime, service objectives, and high-risk server-key/escrow/public-trust features explicit.
