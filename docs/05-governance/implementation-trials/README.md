...
The [rustils filesystem composed evidence trial proposal](rustils-trial-proposal.md) (`TRIAL-0002`) is a second application, citing an existing external repository ([`baileyrd/rustils`](https://github.com/baileyrd/rustils)) as candidate qualified input evidence, and the **first trial this governance model has actually authorized and closed** (revision 5, 2026-08-10 — `Authorized` → `Active` → `Closed` within one authorized session, per the Bounds gate's own single-session limit). It got there in five revisions: revision 1 narrowed the subject to filesystem alone once that domain's promotion review reached an accepted Experimental decision; revision 2 staffed Ownership and defined Bounds; revision 3 bound Repository to rustils' now-Accepted standards profile and reviewed Cross-cutting; revision 4 bound the Verification protocol, reviewed Learning value, and — with RFC-0004 resolving Ownership's independence question — cleared every entry gate, authorizing the trial; revision 5 executed the authorized comparison work and closed it, producing the [comparison record](../../02-capabilities/filesystem/rustils-comparison.md) (`RT-001`/`RT-002` both disposed Inconclusive, each with a specific named evidence gap) and an explicit no-change decision for filesystem's promotion status. Closure does not itself establish any capability's Stable maturity, API, or product status (`RM-TRIAL-MODEL-0002`) — it demonstrates the governance model working end-to-end on a real, bounded, low-risk subject.
