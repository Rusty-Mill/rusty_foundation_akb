...
| Field | Value |
|---|---|
| Status | **Closed** — 2026-08-10 (Authorized → Active → Closed the same session; see Closeout, below) |
| Revision | 5 — comparison record published, both hypotheses disposed, trial closed (see Change log) |
| Owner | baileyrd, per [RFC-0004](../../rfc/0004-solo-maintainer-review-sufficiency.md)'s solo-maintainer sufficiency rule (see Named ownership, below) |
| Reviewers | baileyrd: architecture, filesystem capability, security, evidence — solo, disclosed, sufficient per RFC-0004 |
| Created | 2026-08-10 |
| Authorization expired | Closed before expiry — authorized 2026-08-10, would have expired 2026-09-09; the bounded work (one comparison-record entry per `RT-001`/`RT-002`) completed in the same authorized session, per the Bounds gate's own "produced and published in a single authorized session" limit |
| Implementation authority | None throughout — this trial's authorization permitted, and its closed work consisted of, exactly the bounded, read-only comparison work in Scope, limits, and nonclaims, below. No rustils dependency, provider call, native/unsafe code, credential, benchmark run, release, or product-repository work occurred |
...
**Candidate repository generation:** `baileyrd/rustils` at commit `b8bb862091e8af8dc484f8a8f2c4e76f54d37f16` (main, as of rustils#123), carrying an **Accepted** [repository standards profile](https://github.com/baileyrd/rustils/blob/main/docs/rusty-mill-profile.md) v0.2.0, scoped exactly to the filesystem domain and bound to the R2/D2 work in rustils#122. Four disclosed gaps (unsafe budget, RustSec advisory scanning, Miri-in-CI, performance baselines) are tracked there as formal, owned, expiring exceptions (`EXC-RUSTILS-0001`–`0004`), not blockers to this Accepted status — none of them weaken a foundation safety, authority, evidence, or release rule.
...
### Verification protocol (`entry-gates.md`'s Verification row: assertions, executable cases, benchmark scenarios, comparison rules, evidence location)

**Assertions bound.** `RT-002` binds to [`rm.assertion.filesystem.resolve@1`](../../02-capabilities/filesystem/traceability.md) ("directory-relative authority, typed policy, R-level containment, race resistance, access non-escalation, cleanup, and redacted diagnostics"). `RT-001` binds to the same assertion — `resolve.md`'s own scope covers the path-representation question RT-001 asks (the AKB's `path-model` material is a chapter within `resolve.md`'s assertion, not a separately identified one). No `RT-00N` hypothesis here touches `rm.assertion.filesystem.durability@1` — this trial's subject was narrowed to the resolution/path questions `RT-001`/`RT-002` ask, not a durability comparison; a `write_atomic`/D2 hypothesis is a candidate for a future trial revision, not manufactured here to pad Verification.

**Executable cases cited (rustils' own, not re-executed under this trial's authority):**

| Case | Backend | What it pins |
|---|---|---|
| `linux_open_dir_rejects_a_symlink_in_an_intermediate_component` | Linux | `RT-002`'s Supporting-observation claim: intermediate-symlink rejection under `openat2` `RESOLVE_NO_SYMLINKS` |
| `linux_create_dir_rejects_a_symlink_in_an_intermediate_component` | Linux | Same claim, for `create_dir`'s parent-resolution path |
| `windows_open_dir_rejects_a_reparse_point_in_an_intermediate_component` | Windows | `RT-002`'s Supporting-observation claim, Windows half: intermediate-reparse-point rejection under `OBJ_DONT_REPARSE` |
| `windows_create_dir_rejects_a_reparse_point_in_an_intermediate_component` | Windows | Same claim, for `create_dir` |

All four live in rustils' committed parity suites (`crates/platform-linux/tests/parity.rs`, `crates/platform-windows/tests/parity.rs`) at the bound commit and ran green in rustils' own CI (rustils#122) — cited as candidate input evidence per RFC-0002, not re-executed here (Operations: Not applicable, below).

**Benchmark scenarios: none, reviewed non-applicable** — consistent with Cross-cutting's Performance row, above, and with filesystem's own promotion review excluding any native-performance claim for this R2/D2 slice.

**Comparison rules:** the `RT-00N` hypothesis table's own Supporting observation / Refuting observation / Inconclusive condition columns, above, *are* the comparison rules — a disposition of "supported" requires the exact supporting-observation condition to hold against the cited executable cases and no refuting-observation condition to hold; "refuted" requires the reverse; anything not cleanly resolved by those conditions (e.g. the untested Linux mount-crossing half) is "inconclusive," not rounded to either side.

**Evidence location (bound, not "candidate"):** the comparison record publishes to `docs/02-capabilities/filesystem/rustils-comparison.md`, linked from `promotion-review.md`'s existing "Candidate R/D-level evidence" section (which already exists and already cites the same rustils commit and PR, so the comparison record's job is to formalize the `RT-00N` disposition explicitly, not duplicate evidence already present there).

## Repository and operations

No repository is authorized or selected under this trial's own authority. `baileyrd/rustils` remains an externally, independently governed repository throughout the Proposed and (if reached) Blocked-to-Authorized states; this trial does not gain write access, CI trust, or operational authority over it. Now that authorization has been granted (see Gate review and decision, below), the bound generation is: rustils commit `b8bb862091e8af8dc484f8a8f2c4e76f54d37f16`, reviewer baileyrd (per Named ownership), and evidence location `docs/02-capabilities/filesystem/rustils-comparison.md` (bound in the Verification protocol, above), linked from `promotion-review.md`.
...
`entry-gates.md`'s Cross-cutting row asks for security, privacy, accessibility, i18n, observability, performance, and operational review *plans* — not independent execution of them, unlike Ownership. This trial's entire authorized scope (Scope, limits, and nonclaims, above) is read-only document comparison: no code runs, no provider is called, no secret or credential is touched. Each dimension is reviewed here rather than silently assumed, per `entry-gates.md`'s own header ("a reviewed `not-applicable` ... is not the same as missing evidence"):

| Dimension | Plan | Applicability |
|---|---|---|
| Security | Read-only fetch of rustils' public repository at a bound commit; no execution, no secrets, no network beyond that fetch and this AKB's own repository. The only artifact produced is the comparison record itself, whose accuracy risk is mitigated by the Evidence plan's exact-citation requirement, above — not a security control, but the actual failure mode here (a wrong citation) isn't a security one | Reviewed: no code-execution attack surface exists to plan against |
| Privacy | No personal data is read, stored, or produced — rustils is a systems library with no user data; the comparison record cites source code and documentation only | Reviewed non-applicable |
| Accessibility | Output is Markdown text following this AKB's own existing conventions (headings, tables, no color-only signaling) — the same posture as every other document in this repository; no distinct concern this trial introduces | Reviewed: inherits the AKB's own documentation accessibility posture, not a new plan |
| Internationalization | English only, matching this AKB's existing convention (not itself localized) | Reviewed non-applicable |
| Observability | No runtime system exists to instrument. The trial's own process is observable through this document's Change log (every revision recorded) and, once authorized, the Closeout section (every hypothesis's disposition recorded) — self-documenting by the trial-record format itself, not a separate telemetry plan | Reviewed: satisfied by the trial-record structure already in place |
| Performance | No execution, no benchmark, matching the Operations gate's own "no code execution" finding below | Reviewed non-applicable |
| Operational | Publication path already bound in Repository and operations, above (a comparison-record file per subject domain, linked from that domain's `promotion-review.md`); no CI, runner, secret-injection, or artifact-retention plan is needed beyond that, consistent with Operations being `Not applicable` | Reviewed: covered by Repository and operations, not duplicated here |

None of these are silent skips — each has a stated reviewer and rationale, satisfying `RM-TRIAL-ENTRY-0001`'s "exact evidence and reviewer" requirement for a gate whose own schema doesn't demand independent execution, only a reviewed plan.
...
| Learning value | **Pass** (as of this revision) | `RT-001`/`RT-002` are falsifiable and cite exact rustils source and exact accepted ADRs; `RT-002` specifically now tests the *evidenced* R2 subset against ADR-0007, not a hypothetical one. Reviewed: implementation-level comparison is justified over further specification work because the AKB's filesystem contracts (`resolve.md` et al.) are already Draft-complete text — what's missing is checking real native evidence against them, which requires reading rustils' actual code/tests (an empirical question), not writing more specification prose | Architecture and evidence reviewers: baileyrd (per RFC-0004) | reviewed and bound to this exact `RT-00N` set; a materially different hypothesis set requires its own review |
...
| Ownership | **Pass** (as of this revision) | trial owner and every applicable reviewer role (architecture, filesystem capability, security, evidence) named — baileyrd. Per [RFC-0004](../../rfc/0004-solo-maintainer-review-sufficiency.md), solo staffing satisfies the independence expectation itself (`TRIAL-0002-W002` is superseded, not overridden — see Named ownership, below); the substantive review work this gate's own qualification previously flagged as outstanding is now done (Learning value and Verification, this revision) | Authorizing maintainer: baileyrd | bound to RFC-0004's standing rule; reactivates the independence requirement automatically the moment a second accountable person is named for any reviewer role, per RFC-0004's own reactivation trigger — no separate action needed here |
| Repository | **Pass** (as of revision 3) | rustils' standards profile (`docs/rusty-mill-profile.md` v0.2.0) is Accepted, scoped to the filesystem domain and bound to commit `b8bb862091e8af8dc484f8a8f2c4e76f54d37f16` (rustils#123) — source, architecture, toolchain, targets, dependencies, and exceptions are all bound per `entry-gates.md`'s Repository row; the four disclosed gaps are tracked as formal exceptions there, not silent | Standards reviewer: baileyrd | bound to that exact profile generation; a materially different profile revision requires its own materiality review here too |
| Verification | **Pass** (as of this revision) | assertions (`rm.assertion.filesystem.resolve@1`), executable cases (four named parity tests, both backends), benchmark scenarios (none, reviewed non-applicable), comparison rules (the `RT-00N` table's own Supporting/Refuting/Inconclusive columns), and evidence location (`docs/02-capabilities/filesystem/rustils-comparison.md`, bound not candidate) are all defined — see Verification protocol, above | Evidence reviewer: baileyrd | bound to this exact assertion/case/location set; a materially different set requires its own review |
| Cross-cutting | **Pass** (as of this revision) | security/privacy/accessibility/i18n/observability/performance/operational review plans written and reviewed (Cross-cutting review, above) — every dimension reviewed as applicable or reviewed-not-applicable, none silently skipped; `entry-gates.md`'s own schema asks for plans, not independent execution, so this gate (like Bounds and Repository) has no independence ceiling | Trial owner: baileyrd | bound to this exact revision's scope (read-only, no execution) — a materially different trial scope requires its own review |
...
**Decision: Authorized.** Every applicable gate is `Pass`; `Operations` is reviewed `Not applicable`. Per `RM-TRIAL-ENTRY-0002`, authorization requires no gate to be failed, unknown, expired, contradictory, or aggregate-score-only — none is. This binds exactly: architecture model 1.99.0; filesystem's Experimental promotion decision (2026-08-10); rustils commit `b8bb862091e8af8dc484f8a8f2c4e76f54d37f16`; rustils-profile v0.2.0 and its exceptions `EXC-RUSTILS-0001`–`0004`; hypotheses `RT-001`/`RT-002`; the Bounds, Verification protocol, and Cross-cutting plan above; owner/reviewer baileyrd under RFC-0004; and an expiry of 2026-09-09 or material drift, whichever comes first (`RM-TRIAL-ENTRY-0003`, `RM-RUSTILS-TRIAL-0002`). This authorization permits exactly the bounded read-only comparison work in Scope, limits, and nonclaims — nothing broader — and does not itself establish any capability's Stable maturity, API, or product status (`RM-TRIAL-MODEL-0002`).
...
| 3 | 2026-08-10 | Rebased the candidate repository generation to `b8bb862091e8af8dc484f8a8f2c4e76f54d37f16` (rustils#123, merged) — rustils' standards profile is now Accepted v0.2.0, filesystem-scoped. Wrote and reviewed the Cross-cutting security/privacy/accessibility/i18n/observability/performance/operational plan (every dimension reviewed as applicable or reviewed-not-applicable for this trial's read-only scope). Repository moved Qualified → Pass (a real bound generation now exists, not a pending PR); Cross-cutting moved Unknown → Pass (plans written and reviewed; that gate's schema asks for definition, not independent execution). Merged main separately brought RFC-0004 (solo-maintainer review sufficiency), which supersedes `TRIAL-0002-W002` without changing Ownership's Qualified outcome. Every other gate reused unchanged per `RM-TRIAL-CHANGE-0003` | Repository + Cross-cutting gates only; still `Not authorized` overall — Learning value, Verification, and Ownership remain `Qualified` |
| 4 | 2026-08-10 | Bound the Verification protocol precisely: assertions (`rm.assertion.filesystem.resolve@1`), four named executable cases across both backends, no benchmark scenarios (reviewed non-applicable), comparison rules (the `RT-00N` table's own columns), and a bound (not candidate) evidence-publication location. Reviewed Learning value explicitly (implementation-level comparison justified over further specification work). With RFC-0004 already resolving Ownership's independence question and this revision completing the substantive review work the previous revision's Ownership row flagged as outstanding, Ownership moves to `Pass`. Learning value, Verification, and Ownership all moved to `Pass` — every applicable gate now clears. **Authorized this revision**, expiring 2026-09-09 or on material drift | Learning value, Verification, and Ownership gates; trial moves `Proposed` → `Authorized` |
| 5 | 2026-08-10 | Executed the authorized comparison work and closed the trial in the same session, per the Bounds gate's single-session limit. Published the [comparison record](../../02-capabilities/filesystem/rustils-comparison.md): `RT-001` disposed Inconclusive (no executable case in rustils' own suite exercises the hard non-UTF-8/lone-surrogate path cases the claim depends on); `RT-002` disposed Inconclusive (link-confinement half fully test-verified on both backends; mount-crossing half requested in code but not test-verified, matching the hypothesis's own bound inconclusive condition exactly). Named the follow-on decision: explicit no-change to filesystem's Experimental promotion status, with two non-binding suggestions recorded for rustils. Linked the comparison record from `promotion-review.md`. No resources/authority/credentials ever existed under this trial's authority (Operations: Not applicable throughout) — nothing to inventory or revoke at close | Trial moves `Authorized` → `Active` → `Closed`; filesystem's promotion status unchanged (explicit no-change decision, `RM-RUSTILS-TRIAL-0004`) |

## Closeout

**Closed 2026-08-10.** Every Closeout requirement is met:

- **Hypothesis dispositions.** Both `RT-001` and `RT-002` disposed **Inconclusive**, with exact citations, in the published [comparison record](../../02-capabilities/filesystem/rustils-comparison.md) — `RT-002` substantially supported for the test-verified link-confinement scope with one specific, named gap (Linux mount-crossing containment requested but not test-verified); `RT-001` has no executable evidence in rustils' own suite either way, distinguished explicitly from Rust std's own separate, external `OsString` guarantee. Per `RM-TRIAL-MODEL-0003`, inconclusive is an equally valid, evidence-preserving outcome — neither hypothesis was forced toward supported or refuted to manufacture a cleaner-looking close.
- **Comparison record published.** At `docs/02-capabilities/filesystem/rustils-comparison.md`, linked from `promotion-review.md`'s "Candidate R/D-level evidence" section (this same change).
- **Follow-on decision named.** Explicit **no-change decision** for filesystem's Experimental promotion status (recorded in the comparison record and in `promotion-review.md`) — neither disposition refutes ADR-0006/0007 or the bound R2/D2 scope, and per `RM-RUSTILS-TRIAL-0004` a comparison finding cannot change promotion status by itself regardless. Two non-binding suggestions for rustils (a hard-case path round-trip test; a bind-mount fixture) are recorded as informative findings only — this trial has no authority to request them.
- **Resource/authority inventory.** Nothing to inventory. This trial's `Operations` gate was `Not applicable` throughout (Gate review and decision, above) — no generated authority, resource, process, credential, secret, or host mutation ever existed to account for, sanitize, or revoke.
- **Residuals and limitations.** The two named verification gaps (`RT-001`'s missing round-trip test; `RT-002`'s missing mount-crossing test) are the trial's residual, recorded findings — not defects in this trial's own process, but limits of what rustils' own current evidence supports.

Per the state diagram above, this trial moved `Authorized` → `Active` → `Closed` within the same authorized session, consistent with the Bounds gate's own "produced and published in a single authorized session" limit — there was no multi-day Active period requiring a separate transition record.
