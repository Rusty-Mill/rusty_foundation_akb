...
| Field | Value |
|---|---|
| Status | Proposed; authorization blocked. **Superseded in practice by [ADR-0166](../../adr/0166-knowledge-qualifies-for-fast-lane-entry.md)** — `knowledge` qualifies for [RFC-0005](../../rfc/0005-fast-lane-implementation-entry.md)'s fast lane, so implementation proceeds in `rusty_knowledge` without waiting for this trial to reach Authorized. This record is retained as history, not deleted. |
| Revision | 0 |
...
| 0 | 2026-08-10 | RFC-0005 (fast-lane implementation entry) and ADR-0166 accepted: `knowledge` qualifies (no unsafe/FFI authorship, no native platform backend, no authority/credential/crypto primitives), so implementation proceeds in `rusty_knowledge` without waiting for this trial's Subject gate to clear. This record is retained as history and superseded in practice, not deleted | This trial's own entry process no longer gates `knowledge`'s implementation — see ADR-0166 |
