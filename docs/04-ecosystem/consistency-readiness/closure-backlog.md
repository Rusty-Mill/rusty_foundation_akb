...
| CR-008 | Medium | Draft-to-Experimental evidence requires continuing domain review. | The generated [promotion scorecards](promotion-scorecards.md) now preserve pass/fail/unknown and explicit non-promotion conclusions; close after every required gate has exact reviewed evidence. | In progress |
| CR-009 | High | Foundation-wide software development standards are not yet defined. | Accepted standards covering Rust/API design, unsafe-code policy, errors, async/sync behavior, testing, performance, security, accessibility/i18n, observability, documentation, dependencies/supply chain, review, CI, release evidence, and governed exceptions before implementation trials. | Open |
