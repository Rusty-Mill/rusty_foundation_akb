...
The [PKI-issuance foundations](../02-capabilities/security/pki-issuance-README.md) define a separate lifecycle from typed enrollment intent through proofing and enrollment authority, opaque-key proof of possession, immutable request, CA policy construction and durable signing transaction, protocol delivery, verified key-bound installation, consumer activation, renewal/rekey/replacement, revocation, reconciliation, and CA recovery. These stages retain distinct evidence: a valid request or attestation is not issuance authority; issuance is not installation or activation; renewal guidance is not execution authority; and certificate acceptance is not identity or application authorization ([ADR-0090](../adr/0090-certificate-requests-prove-key-possession-not-issuance-authority.md), [ADR-0091](../adr/0091-renewal-creates-a-new-credential-generation-with-explicit-continuity.md)). Its specifications remain Draft and cannot amend this model.

The [authority-unit composition](../02-capabilities/security/authority-dependencies.md) governs typed principal/claim/context evidence, explicit authority-bearing resources, policy evaluation, operation-time native enforcement, multidimensional attenuation, audience-bound transactional delegation, and alias-aware expiry/revocation as separate stages. Identity, possession, membership, entitlement, policy permit, access-query success, transfer acceptance, audit publication, and prior success do not create unrelated or future authority. Derivation and redelegation monotonically narrow effective authority and depth; close, expiry, revocation request, revocation observation, in-flight work, and committed effects remain distinct. The consuming resource domain owns object identity, operation milestones, effects, and reconciliation ([ADR-0009](../adr/0009-identity-is-not-authority.md), [ADR-0010](../adr/0010-native-operation-is-the-authorization-point.md), [ADR-0162](../adr/0162-attenuation-enforcement-is-a-scoped-claim-vector-not-a-security-score.md)). These specifications remain Draft and cannot amend this model.
...
The [windowing promotion review](../02-capabilities/windowing/promotion-review.md) is the first schema-scaled native UI-domain candidate. It adds semantic benchmark identities, exact Win32/Wayland/X11/AppKit source frontiers, cross-cutting review, ownership/trial bounds, and a visual [composition register](../02-capabilities/windowing/dependencies.md). Windowing remains Draft. Request acceptance, committed state, surface readiness, graphics presentation, displayed frame, focus, accessibility exposure, and capture protection remain separate evidence milestones; no provider, graphics API, or implementation is selected.
...
Cryptography and key management form one governed promotion unit across versioned workload policy, opaque keys, typed operation families, transfer/serialization, providers/hardware/remote boundaries, attestation/certification, conformance, lifecycle, and performance. This shared unit does not create a generic operation: hash, MAC, derivation, password derivation, AEAD, signature, verification, agreement, encapsulation, wrapping, import/export, and provider-mediated use retain distinct exact contracts and evidence. Policy resolves to an immutable plan before provider activation or key/operation creation; installed catalogs, defaults, “best available,” OS identity, hardware labels, and certification names cannot substitute or strengthen the plan ([ADR-0081](../adr/0081-cryptographic-policy-precedes-provider-selection.md)).

PKI validation composes bounded parsing, immutable provenance-bearing trust snapshots, bounded path construction, exact path/purpose/time/algorithm validation, consumer-profile identity matching, status/network/cache evidence, and dependency-expiring trust results. Presented certificates remain untrusted candidates rather than a chain ([ADR-0082](../adr/0082-presented-certificates-are-candidates-not-a-chain.md)). A successful result is context-bound evidence, not identity, proof of private-key possession, account mapping, authorization, issuance correctness, or action authority ([ADR-0083](../adr/0083-trust-results-are-context-bound-evidence-not-identity-or-authority.md)). Standards claims bind the applicable base, updates, errata, algorithms, and consumer profile rather than naming one RFC generically.
