...
The [security registry](../02-capabilities/security/promotion-units.md) defines seven Draft units: authority, restricted execution, secure randomness, secret protection, cryptography/key management, PKI validation, and certificate issuance. It links complete unit dossiers separately from primary specifications; the generated index verifies existence plus canonical status, exact subject, architecture frontier, and explicit implementation nonauthorization, but cannot infer semantic completeness or maturity. Secure-random, restricted-execution, secret-protection, and cryptography/key-management have schema-valid unit dossiers, while attenuation remains partial evidence inside the broader authority unit. No dossier changes maturity or authorizes trials/code.
